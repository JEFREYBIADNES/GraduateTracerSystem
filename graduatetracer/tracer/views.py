...
def AdminProfilePicture(request):
    user = request.user
    admin_info = Profile(instance=user)

    if request.method == 'POST':
        profile_picture = request.FILES['profile']
        admin_info = Profile(request.POST, instance=user)
...
@login_required(login_url='login')
@allowed_users(allowed_roles=['is_admin_sao'])
def AdminDisplayPendingAccts(request):
    gradAccts = User.objects.all()

    context = {'gradAccts': gradAccts}
    return render(request, 'tracer/admin/displayPending.html', context)


@login_required(login_url='login')
@allowed_users(allowed_roles=['is_admin_sao'])
def AdminDisplayApprovedAccts(request):
    gradAccts = User.objects.all()

    context = {'gradAccts': gradAccts}
    return render(request, 'tracer/admin/displayApproved.html', context)
...
def ApproveUser(request, pk):
    ...
    return redirect('AdminDisplayPendingAccts')
...
def user_information(request, pk):
    ...
    return render(request, 'tracer/admin/user_info.html', context)
